...
namespace ns3 {
    ...
    void Pipeline::AddPush (int user, long long rank, int token) {
        ...
        //cout << "add user: " << user << " token: " << token << endl;
    ...
    int Pipeline::GetToken () {
        ...
        buffer.pop();
        // cout << "buffer num--: " << buffer_num << endl;
        // cout << "gettoken: " << ret << endl;
        return ret;
    }

    bool Pipeline::CheckRealQueue(int id) {
        // Hadoop queue or flow queue is real queue
        // if (id >= 2)
        //     return 1;
        //cout << "id: " << id << endl;
    ...
    void Pipeline::WakeUp () {
        ...
        for (int i = 0, j; i < N; ++i) {
            ...
            // Inherit task from the last RPU
                ...
                RPU[i] = nt[j ^ 1];
                //cout << "RPU[i].root inheret: " << RPU[i].root << endl;
            }
        ...
        for (int i = 0, j; i < N; ++i) {
            ...
            if (RPU[i].type == Empty && !locked[i]) {
                if (!operation_list[i].empty()) {
                    ...
                    if (t.type == Push) {
                        RPU[i] = t;
                        //cout << "RPU[i].root push: " << RPU[i].root << endl;
                        ...
                        if (RPU[j].TTL <= 1) {
                            RPU[i] = t;
                            //cout << "RPU[i].root pop: " << RPU[i].root << endl;
                            SendOperation(i);
