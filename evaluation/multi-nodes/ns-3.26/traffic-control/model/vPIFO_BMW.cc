...
{
    class Node;
    const int vPIFO_BMW::ROOT = 5;
    //const int vPIFO_BMW::SIZE = ((1 << 10) - 1) / 3;
    const int vPIFO_BMW::SIZE = 2600;
    
    NS_OBJECT_ENSURE_REGISTERED(vPIFO_BMW);
    TypeId vPIFO_BMW::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::vPIFO_BMW")
			.SetParent<QueueDisc>()
			.SetGroupName("TrafficControl")
			.AddConstructor<vPIFO_BMW>();
		return tid;
	}

    bool vPIFO_BMW::IsHadoop(int x)
    {
        return (x % 10) > (x / 10);
    }
    ...
    std::string vPIFO_BMW::GetFlowLabel(Ptr<QueueDiscItem> item)
    {
    ...
    void vPIFO_BMW::InitializeParams(void) {
        // flow_label
    ...
    bool vPIFO_BMW::DoEnqueue(Ptr<QueueDiscItem> item)
    {
        // TenantTag my_tag;
        ...
        if(!newflow_mp[flow_id]){       //新流建立，加入BMW-Tree。其余的都先只加入队列，不进入BMW-Tree
            ...
            if(flow_id == 11){
                ...
                pipe.AddPush(ROOT, rk1, 1); //之所以token是1，是因为1代表了下一级的sche_tree索引，方便pipeline代码中addpop
    ...
    Ptr<QueueDiscItem> vPIFO_BMW::DoDequeue()
    {
        ...
        if(ans == 12 || ans == 13){ //flow bc
            //wfq出包
            //流程：先按照schedule算法出包，然后实际出队，然后选择一个包进入BMW-Tree（两种方法的不同之处在实际出队的地方体现，即出去一个包后到底选什么包入packet_buffer）
            ...
            sch_tree[0]->Dequeue(rpacket_size);
    ...
    Ptr<const QueueDiscItem> vPIFO_BMW::DoPeek(void) const {
        return 0;
    }
    
    bool vPIFO_BMW::CheckConfig(void) {
        return 1;
    }
