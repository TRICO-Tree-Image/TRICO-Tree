...
{
    class Node;
    const int vPIFO_wrong::ROOT = 5;
    //const int vPIFO_wrong::SIZE = ((1 << 10) - 1) / 3;
    const int vPIFO_wrong::SIZE = 2600;
    
    NS_OBJECT_ENSURE_REGISTERED(vPIFO_wrong);
    TypeId vPIFO_wrong::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::vPIFO_wrong")
			.SetParent<QueueDisc>()
			.SetGroupName("TrafficControl")
			.AddConstructor<vPIFO_wrong>();
		return tid;
	}

    bool vPIFO_wrong::IsHadoop(int x)
    {
        return (x % 10) > (x / 10);
    }

    vPIFO_wrong::vPIFO_wrong()
    {
        flow_map.clear();
        sch_tree.clear();
        queue_map.clear();
    ...
    std::string vPIFO_wrong::GetFlowLabel(Ptr<QueueDiscItem> item)
    {
    ...
    void vPIFO_wrong::InitializeParams(void) {
        // flow_label
    ...
    bool vPIFO_wrong::DoEnqueue(Ptr<QueueDiscItem> item)
    {
        // TenantTag my_tag;
    ...
    Ptr<QueueDiscItem> vPIFO_wrong::DoDequeue()
    {
        ...
        if(flowid == 1){    //flowid为1，代表是reference
            ...
            //此为错误版本，哪个流出数据包，就从哪个流队列中选数据包进入pifo
    ...
    Ptr<const QueueDiscItem> vPIFO_wrong::DoPeek(void) const {
        return 0;
    }
    
    bool vPIFO_wrong::CheckConfig(void) {
        return 1;
    }
