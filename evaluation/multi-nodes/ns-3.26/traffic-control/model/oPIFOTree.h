#ifndef OPIFOTREE_H
#define OPIFOTREE_H

#include <iostream>
#include <vector>
#include <queue>
#include <unordered_map>
#include <deque>
#include <fstream>
#include <sstream>
#include <string>
#include <algorithm>
#include <stdexcept>
#include <regex>
#include <map>
#include <memory>
#include "ns3/ipv4-queue-disc-item.h"
#include "ns3/node.h"
#include "ns3/ptr.h"
#include "ns3/tcp-header.h"
#include "ns3/Tenant-tag.h"
#include "MyPipeline.h"
#include "MyScheduling.h"
#include "FlowScheduler.h"
#include <unordered_set>

namespace ns3 {

class oPIFOTree : public QueueDisc {
public:
    const int BUFFER_SIZE = 104857600;
    const double ALPHA = 14;
    const std::string SCHEDULING_ALGORITHM = "exp2.txt";
    // int cnt = 0;

    struct Pkt {
    public:
        Ptr<QueueDiscItem> pktid;
        int flowid;
        int length;
        int flow_size;

        Pkt(Ptr<QueueDiscItem> pktid, int flowid, int length, int flow_size);
    };

    struct FlowQueueItem {
    public:
        int flowid;
        Pkt pkt;
        std::vector<double> rank; // Rank for each layer
        int layercnt; // Layer count

        FlowQueueItem(int flowid, Pkt pkt, int layercnt);
    };

    struct PifoItem {
    public:
        double rank;
        Pkt pkt;
        int flowid;
        int pifoid;
        bool is_token;

        PifoItem(double rank, Pkt pkt, int flowid, int pifoid, bool is_token = false);

        bool operator<(const PifoItem& other) const;
    };

    class PIFO {
    public:
        int pifoid;
        std::priority_queue<PifoItem> pifo_queue; // Priority queue based on the rank
        std::string schedule_algorithm;
        std::vector<std::tuple<int, bool, int>> children_node; // [(flowid/pifoid, is_pifo, weight/priority)]
        std::unordered_map<int, double> finish_time; // Finish time per flow/child
        double virtual_time;


        PIFO();
        PIFO(int pifoid, std::string schedule_algorithm, std::vector<std::tuple<int, bool, int>> children_node);

        double rank_cal(Pkt pkt, int children_node_id);
        void after_pop(PifoItem item);
    };

    class PifoTreeScheduler {
    public:
        std::unordered_map<int, std::vector<int>> treepath; // flowid -> related PIFO ids
        std::unordered_map<int, std::deque<FlowQueueItem>> queue_map; // pifoid -> queue of FlowQueueItems
        std::unordered_map<int, bool> flow_in_pifo; // Judge if flow is in PIFO
        std::unordered_map<int, PIFO> pifo_map;
        int root;
        int buffer_size;
        double alpha;
        int rest_buffer_size;
        std::unordered_map<int, int> queue_length;

        PifoTreeScheduler();

        void initialize_params(const std::vector<std::tuple<int, std::string, std::vector<std::tuple<int, bool, int>>>>& pifo_config, const std::unordered_map<int, std::vector<int>>& treepaths, int root);
        void doenqueue(Pkt pkt);
        void push_to_pifo(int flowid);
        Pkt dodequeue();
        void PrintContainerSizes();
    };

    PifoTreeScheduler scheduler;

    oPIFOTree();
    static TypeId GetTypeId(void);
    void InitializeParams(void);
    bool DoEnqueue(Ptr<QueueDiscItem> item);
    Ptr<QueueDiscItem> DoDequeue();
    Ptr<const QueueDiscItem> DoPeek(void) const;
    bool CheckConfig(void);
    int GetFlowLabel(Ptr<QueueDiscItem> item);

private:
    void ReadScheduleAlgo(const std::string& filename, PifoTreeScheduler& scheduler);
    std::ofstream outfile;
};

} // namespace ns3

#endif // OPIFOTREE_H