...
class PIFO:
    ...
    def after_pop(self, item: PifoItem):
        if self.schedule_algorithm == "WFQ":
            ...
            # self.virtual_time = max(self.virtual_time, self.finish_time[item.pifoid])
...
class PifoTreeScheduler:
    ...
    def doenqueue(self, pkt: Pkt):
        ...
        self.queue_map[flowid].append(item)
    ...
    def push_to_pifo(self, flowid):
        ...
        for i in range(len(self.treepath[flowid])):
            ...
            next_pifo_id = self.treepath[flowid][i + 1] if i + 1 < len(self.treepath[flowid]) else flowid
            rank = item.rank[i]
            pifo_item = PifoItem(rank, item.pkt, flowid, next_pifo_id)
            heapq.heappush(self.pifo_map[current_pifo_id].pifo_queue, pifo_item)
            # print(f"Flow {flowid} pushed to PIFO {current_pifo_id} with rank {rank}.")

        self.flow_in_pifo[flowid] = True

    def dodequeue(self):
        rootPifo = self.pifo_map[self.root]
        if not rootPifo.pifo_queue:
            return None
        item = heapq.heappop(rootPifo.pifo_queue)
        rootPifo.after_pop(item)
        # if rootPifo.schedule_algorithm == "WFQ":
        #     with open("oPIFOTree_virtual_time.txt", "a") as file:
        #         file.write(
        #             f"Flow {rootPifo.pifoid} 's virtual time: {rootPifo.virtual_time} \n")
        while True:
        ...
        # with open("oPIFOTree_Pop_packet.txt", "a") as file:
        #     file.write(f"Dequeued packet {item.pkt.pktid} from PIFO {item.pifoid} and Flow {item.flowid}.\n")
        # print(f"Dequeued packet {item.pkt.pktid} from PIFO {item.pifoid} and Flow {item.flowid}.\n")
        if len(self.queue_map[item.flowid]):
            self.push_to_pifo(item.flowid)
            # print(f"Dequeued packet {item.pkt.pktid} from PIFO {item.pifoid} and Flow {item.flowid}.")
        return item.pkt
...
def main():
    ...
    for x1 in range(1, 11):
        scheduler.doenqueue(Pkt(pktid=x1, flowid=3, length=10, flow_size=10))

    # Flow 5 packets (with pktid from 11 to 20)
    for x2 in range(11, 21):
        scheduler.doenqueue(Pkt(pktid=x2, flowid=5, length=10, flow_size=10))

    # Flow 4 packets (pktid 21 and 22)
    scheduler.doenqueue(Pkt(pktid=21, flowid=4, length=30, flow_size=20))
    scheduler.doenqueue(Pkt(pktid=22, flowid=4, length=30, flow_size=20))

    # Step 3: Save the current PIFO tree and queue states to a JSON file
    # scheduler.save_pifo_tree_to_file("oPIFO_after_push.json")
    ...
    # scheduler.save_pifo_tree_to_file("oPIFO_after_pop.json")
