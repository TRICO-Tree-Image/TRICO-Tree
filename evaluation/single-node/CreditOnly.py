...
class PifoTreeScheduler:
    ...
    def packets_to_send_empty(self):
        ...
        if self.packets_to_send:
            ...
            # with open("Creditonly_Pop_packet.txt", "a") as file:
    ...
    def dodequeue(self):
        ...
        while not self.packets_to_send:
            ...
            if rootPifo.pifo_queue:
                ...
                # with open("Creditonly_Pop_packet.txt", "a") as file:
                #     file.write(f"ref in flow {item.flowid} in pifo {item.pifoid}\n")
            else:
                break
            #depth - first search
            self.DFS()

        # with open("Creditonly_Credit.txt", "a") as file:
        ...
        while self.stack:
            ...
            if stackIndex in self.pifo_map:
                ...
                while currentPifo.pifo_queue:
                    ...
                    if currentIndex == -1:
                        break
                    elif item.pifoid in self.flow_in_pifo:
                        break
                    else:
                        self.pifo_map[item.pifoid].credit += item.pkt.length
                    # update the variant and the parameters
...
def main():
    ...
    scheduler.doenqueue(Pkt(pktid=21, flowid=4, length=300, flow_size=30))
    scheduler.doenqueue(Pkt(pktid=22, flowid=4, length=300, flow_size=30))

    # Step 3: Save the current PIFO tree and queue states to a JSON file
    # scheduler.save_pifo_tree_to_file("Creditonly_after_push.json")
    ...
    # scheduler.save_pifo_tree_to_file("Creditonly_after_pop.json")
